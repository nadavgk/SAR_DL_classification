...
class ConvNet(nn.Module):
    def __init__(self, num_classes=17):
        ...
        self.fc1 = nn.Linear(64 * 16 * 16 * 2, 128)  # *2 for concatenated features
    ...
    def forward(self, sen1, sen2):
        ...
        print(f"Sentinel-1 shape after pooling: {x1.shape}")  # Debugging step
        ...
        print(f"Sentinel-2 shape after pooling: {x2.shape}")  # Debugging step
        x2 = x2.view(x2.size(0), -1)  # Flatten

        # Concatenate both branches
        x = torch.cat((x1, x2), dim=1)
        print(f"Concatenated shape: {x.shape}")  # Debugging step
...
def evaluate_model(model, test_loader, criterion, device='cuda'):
    ...
    with torch.no_grad():
        for sen1, sen2, labels in test_loader:
            ...
            correct_predictions += (predicted == labels).sum().item()  # Compare directly
...
train_loader = get_data_loader(r"F:\DS_project\m1613658\random\training.h5", batch_size=64, shuffle=True)
test_loader = get_data_loader(r"F:\DS_project\m1613658\random\testing.h5", batch_size=64, shuffle=False)
